...
class particle_picking(object):
    ...
    def compute(
        ...
        if update_meta_file:
            ...
            if ugraph_shapes is None:
                ugraph_shapes = (
                    np.ones((len(positions), 2)) * self.ugraph_shape
                )
        ...
        if (
            ...
            for ugraph_path in self.ugraph_paths:
                ...
                for (
                    ...
                    self.results_truth["ice_thickness"].append(
                        ice_thickness_value
                    )
            ...
            # loaded and the number of particles per micrograph
            ...
            for ugraph_path in np.unique(
                ...
                positions = np.array(
                    [
                        [
                            ...
                            self.results_truth["position_z"][i],
                        ]
                ...
                ice_thickness_values = [
                    self.results_truth["ice_thickness"][i]
                    for i in range(len(self.results_truth["ice_thickness"]))
                    if self.results_truth["ugraph_filename"][i] == ugraph_path
                ][
                    : self.particles_per_ugraph[
                        self.ugraph_paths.index(ugraph_path)
        ...
        # For each picked particle, check if it is near any of the
        # true particles
        ...
        if update_meta_file:
            ...
            for ugraph_path in np.unique(self.ugraph_paths):
                ...
                # count the number of picked particles close to a
                # true particle (this is the true particle's multiplicity)
    ...
    def _calc_dist_array(self, pos_picked, pos_truth, image_shape):
        # Calculates an array of distances between each pair of
        # truth and picked particles
        r = np.sqrt(
            np.power(
                float(image_shape[0]),
                2,
    ...
    def _calc_neighbours(self, pos_picked, pos_truth, r):
        ...
        # Do the same but this time between the picked particles
        # and the truth particles
    ...
    def compute_precision(
        ...
        """Produces another data frame containing the number of true
        ...
        Args:
            results_picking (pandas.DataFrame): the results of the
            picking algorithm
        ...
        # for each micrograph for each metadata file
        ...
        # FN can be calculated from this by looking at the number of
        # particles with a multiplicity of 0
    ...
    def compute_overlap(
        ...
        """Computes the number of picked particles that overlap with a
         truth particle at a range of radii.
        ...
        Args:
            ...
            results_truth (pandas.DataFrame): the results of the
             truth algorithm
            verbose (bool): print out information about
        ...
        results_picking_grouped = results_picking.groupby(
            ["metadata_filename", "ugraph_filename"]
        )
        results_truth_grouped = results_truth.groupby(
            ["metadata_filename", "ugraph_filename"]
        )
        ...
        keys = results_picking_grouped.groups.keys()
        for key in keys:
            picked_group = results_picking_grouped.get_group(key)
            truth_group = results_truth_grouped.get_group(key)

            picked_pos_x = picked_group["position_x"]
            picked_pos_y = picked_group["position_y"]

            truth_pos_x = truth_group["position_x"]
            truth_pos_y = truth_group["position_y"]
            defocus = truth_group["defocus"].mean()
            ...
            # fixing mypy error, but no idea what/why this is
            # looping over keys and using key[index]
            for i in range(len(r)):
                results["metadata_filename"].append("fix this")
                results["ugraph_filename"].append("fix this")
