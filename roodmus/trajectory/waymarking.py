...
def add_arguments(parser: argparse.ArgumentParser)->argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--digits",
        help=("Number of digits (supplemented with leading zeros) in output file"),
        type=int,
        default=6,
        required=False,
    )
...
def get_trajfiles(trajfiles_dir_path: str, verbose:bool, traj_extension: str='.dcd') -> list[str]:
    ...
    Args:
        ...
        traj_extension (str, optional): File extension of trajectory files. Defaults to '.dcd'.
...
def waymark(
    trajfiles: list[str], topfile: str, rmsd: float, verbose: bool, atom_indices=None,
) -> Tuple[list[list[int]], list[list[np.int64]], 
list[np.ndarray[np.int64,np.int64]], mdt.Trajectory, list[int]]:
    ...
    Args:
        ...
        verbose (bool): Increase output verbosity
        atom_indices (atom_indicesarray_like, optional): Subselection of atoms to use in RMSD calculation. Defaults to None.
    ...
    Returns:
        ...
        List of lists (one per trajectory file) of indexes of waymark-kept conformations,
         List of lists (one per trajectory file) of cumulative count of waymark-kept conformations,
         List of arrays (one per trajectory file), each holding a [1, len(traj)] with rmsd values of reference frame compared to all frames in traj file,
...
def plot_waymarking(waymarking_plots: str, traj_steps: np.uint64, waymarks: list[int], suffix: str=None, xlabel: str="n_frames"):
    """Create and save plots of how the number of waymark-sampled trajectories increases
     as a function of the number of frames or number of trajectory files
    ...
    Args:
        ...
        suffix (str, optional): Descriptive suffix for plot names. Defaults to None.
    ...
    plt.savefig(os.path.join(waymarking_plots, 'waymark_{}.pdf'.format(suffix)))
    plt.savefig(os.path.join(waymarking_plots, 'waymark_{}.png'.format(suffix)))
    plt.clf()
    return

def list_waymark_occupancy(traj_steps: np.uint64, counts: list[int], suffix: str=None):
    ...
    Args:
        ...
        counts (_type_): Cumulative count of waymark-sampled conformations for this trajectory
        suffix (str, optional): Descriptive label for trajectory.. Defaults to None.
    """
    for i_way in np.argsort(counts)[::-1]:
        print('Waymark {} {:2d}: {:5.2f}%'.format(suffix, i_way, counts[i_way] * 100 / traj_steps))
    return

def write_non_redundant_confs(non_redundant_confs: mdt.core.trajectory.Trajectory, output_dir: str, verbose: bool=False, digits: int=6):
    """Write waymark-sampled conformations to disk

    Args:
        non_redundant_confs (mdt.core.trajectory.Trajectory): Sampled conformations
        output_dir (str): Directory to write conformations into
        verbose (bool, optional): Increase output verbosity. Defaults to False.
        digits (int, optional): Digits to use in saved filenames - supplemented by leading zeros. Defaults to 6.
    """
    create_output_dir(output_dir, verbose=verbose)
    for i, traj in enumerate(non_redundant_confs):
        conf_file = os.path.join(output_dir, 'waymark_{}.pdb'.format(str(i).zfill(digits)))
        traj.save(conf_file)
    return

def get_traj_steps(trajfiles: list, topfile: str, verbose: bool)->Tuple[np.uint64, list[np.uint64]]:
    ...
    Returns:
        ...
        list[np.uint64]: Number of frames in each (dcd) file making up trajectory
    """
    traj_steps = 0
    steps_per_file = []
...
def sample_dcd_evenly(trajfiles: list[str], topfile: str, traj_indices: list[list[int]], verbose: bool, output_dir: str, digits: int=6):
    ...
    Args:
        ...
        traj_indices (list[list[int]]): List containing list of indices of conformation(s) in each dcd which need to be saved to file as pdb_
        verbose (bool): Increase output verbosity
    """
    create_output_dir(output_dir, verbose=verbose)
    ...
    for traj_idx, traj_conf_idxs in enumerate(traj_indices):
        ...
        if len(traj_conf_idxs)>0:
            ...
            for i_conf, conf in enumerate(selected_conformations):
                conf_file = os.path.join(output_dir, 'conformation_{}.pdb'.format(str(conformation_counter).zfill(digits)))
                conf.save(conf_file)
                conformation_counter += 1
                if verbose:
                    print('Saved conformation_{}.pdb from traj file {} ({}) to location {}'.format(str(conformation_counter).zfill(digits), traj_idx, trajfile, conf_file))
...
def get_traj_indices(traj_steps:np.uint64, steps_per_file: list[np.uint64], verbose: bool, n_conformations: np.uint64=1, rnd_start: bool=False, seed: int=1385737, contiguous: bool=False)->list[list[int]]:
    """Determine the trajectory frame indices which you want to sample from full trajectory

    Args:
        traj_steps (np.uint64): Total number of frames in entire trajectory
        steps_per_file (list[np.uint64]): List of number of frames per trajectory file (1 entry per file)
        verbose (bool): Increase output verbosity
        n_conformations (np.uint64, optional): Number of conformations to sample. Defaults to 1.
        rnd_start (bool, optional): Whether to start sampling at a random frame. Defaults to False.
        seed (int, optional): Seed to use to generate random starting frame. Defaults to 1385737.
        contiguous (bool, optional): Whether to sample frames contiguously. Defaults to False.

    Returns:
        list[list[int]]: List of lists (one per trajectory file) filled with indices of frames to sample
    """
...
def main(args):
    trajfiles = get_trajfiles(args.trajfiles_dir_path, args.verbose, args.traj_extension)
    ...
    traj_steps, steps_per_file = get_traj_steps(trajfiles, topfile, args.verbose)
    if args.verbose:
    ...
    if args.sampling_method=='even_sampling':
        ...
        traj_indices = get_traj_indices(traj_steps, steps_per_file, args.verbose, n_conformations=args.n_conformations, rnd_start=args.rnd_start, seed=args.random_startpoint_seed, contiguous=args.use_contiguous_conformations)

        # create functio to take list[list[int]] and save the pdbs to file
        sample_dcd_evenly(trajfiles, topfile, traj_indices, args.verbose, args.output_dir)

    if args.sampling_method=='waymark':
        # load initial trajectory
        t = load_traj(trajfiles[0], topfile, args.verbose)
        ...
        # Third is )
        waymarks_list, counts_list, assignments_list, non_redundant_confs, non_redundant_confs_counter_list = waymark(trajfiles, topfile, args.rmsd, args.verbose)
        ...
        # Can not plot waymarking beyond the first traj file but lets do this for first file (highest gradient)
        plot_waymarking(args.waymarking_plots, steps_per_file[0], waymarks_list[0], suffix='singlefile')
        # Unless we only care about file-level granularity it the plotplot_waymarking
        plot_waymarking(args.waymarking_plots, cumulative_steps_per_file[-1], waymarks_list[:][-1], suffix='allfiles', xlabel="n_traj_fles")
        
        if args.verbose:
            # Can not list_waymark_occupancy plot beyond the first traj filebut lets do this for first file
        ...
        write_non_redundant_confs(non_redundant_confs, args.output_dir, args.digits)
    return
