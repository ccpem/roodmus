...
class ExtractStarFile(object):
    ...
    def add_picked_particles(self)->None:
        ...
        # grab the dict that the first ugraph holds and grab the column headings from that
        # ie: this grabs picked_particles["000000"].keys()
        columns = list(self.picked_particles[list(self.picked_particles.keys())[0]].keys())

        loop = particles_loop.init_loop("_rln", columns)

        # now that you have the loop, iterate through all ugraphs and append to a
        ...
        for ugraph_index in self.picked_particles.keys():
            CoordinateX_all_particles.extend(self.picked_particles[ugraph_index]["CoordinateX"])
            CoordinateY_all_particles.extend(self.picked_particles[ugraph_index]["CoordinateY"])
            AutopickFigureOfMerit_all_particles.extend(self.picked_particles[ugraph_index]["AutopickFigureOfMerit"])
            ClassNumber_all_particles.extend(self.picked_particles[ugraph_index]["ClassNumber"])
            AnglePsi_all_particles.extend(self.picked_particles[ugraph_index]["AnglePsi"])
            ImageName_all_particles.extend(self.picked_particles[ugraph_index]["ImageName"])
            MicrographName_all_particles.extend(self.picked_particles[ugraph_index]["MicrographName"])
            OpticsGroup_all_particles.extend(self.picked_particles[ugraph_index]["OpticsGroup"])
            CtfMaxResolution_all_particles.extend(self.picked_particles[ugraph_index]["CtfMaxResolution"])
            CtfFigureOfMerit_all_particles.extend(self.picked_particles[ugraph_index]["CtfFigureOfMerit"])
            DefocusU_all_particles.extend(self.picked_particles[ugraph_index]["DefocusU"])
            DefocusV_all_particles.extend(self.picked_particles[ugraph_index]["DefocusV"])
            DefocusAngle_all_particles.extend(self.picked_particles[ugraph_index]["DefocusAngle"])
            CtfBfactor_all_particles.extend(self.picked_particles[ugraph_index]["CtfBfactor"])
            CtfScalefactor_all_particles.extend(self.picked_particles[ugraph_index]["CtfScalefactor"])
            PhaseShift_all_particles.extend(self.picked_particles[ugraph_index]["PhaseShift"])
    ...
    for line in lines:
        ...
        if len(parse_it)>0:
            if parse_it[0]=='placing':
                ...
                # position
                # orientation 
                particles[str(micrograph_ind).zfill(6)]["molecule"].append(str(parse_it[1]))
...
def load_optics_from_starfile(args, starfile:str, block_name: Optional[str]="optics", loop_name_prefix: Optional[str]="_rln")->dict:
    ...
    if os.path.exists(starfile):
        ...
        """
        optics_dict = {}
        OpticsGroupName = gemmi_block.find_values(loop_name_prefix+"OpticsGroupName")
        ...
        MicrographOriginalPixelSize = gemmi_block.find_values(loop_name_prefix+"MicrographOriginalPixelSize")
        Voltage = gemmi_block.find_values(loop_name_prefix+"Voltage")
        SphericalAberration = gemmi_block.find_values(loop_name_prefix+"SphericalAberration")
        AmplitudeContrast = gemmi_block.find_values(loop_name_prefix+"AmplitudeContrast")
        ImagePixelSize = gemmi_block.find_values(loop_name_prefix+"ImagePixelSize")
        ImageSize = gemmi_block.find_values(loop_name_prefix+"ImageSize")
        ImageDimensionality = gemmi_block.find_values(loop_name_prefix+"ImageDimensionality")

        # don't need to sort this data by ugraph, just fill a list (due to _loop) with single value for each parameter
        ...
        for i in range(len(OpticsGroupName)):
            ...
            optics_dict["MicrographOriginalPixelSize"].append(MicrographOriginalPixelSize.str(i))
            optics_dict["Voltage"].append(Voltage.str(i))
            optics_dict["SphericalAberration"].append(SphericalAberration.str(i))
            ...
            optics_dict["ImageDimensionality"].append(ImageDimensionality.str(i))

        optics_dict["OpticsGroupName"] = np.array(optics_dict["OpticsGroupName"], dtype=str).astype(str).tolist()
        optics_dict["OpticsGroup"] = np.array(optics_dict["OpticsGroup"], dtype=str).astype(int).tolist()
        optics_dict["MtfFileName"] = np.array(optics_dict["MtfFileName"], dtype=str).astype(str).tolist()
        optics_dict["MicrographOriginalPixelSize"] = np.array(optics_dict["MicrographOriginalPixelSize"], dtype=str).astype(float).tolist()
        optics_dict["Voltage"] = np.array(optics_dict["Voltage"], dtype=str).astype(float).tolist()
        optics_dict["SphericalAberration"] = np.array(optics_dict["SphericalAberration"], dtype=str).astype(float).tolist()
        optics_dict["AmplitudeContrast"] = np.array(optics_dict["AmplitudeContrast"], dtype=str).astype(float).tolist()
        optics_dict["ImagePixelSize"] = np.array(optics_dict["ImagePixelSize"], dtype=str).astype(float).tolist()
        optics_dict["ImageSize"] = np.array(optics_dict["ImageSize"], dtype=str).astype(int).tolist()
        optics_dict["ImageDimensionality"] = np.array(optics_dict["ImageDimensionality"], dtype=str).astype(int).tolist()
...
def remove_edge_truth_particles(args, particles: dict)->dict:
    for ugraph_index in particles.keys():
        ...
        acceptance_mask_x = np.logical_and(np.array(particles[ugraph_index]["x"], dtype=float) <= float(args.image_x_pixels) - float(args.remove_edge_particles), np.array(particles[ugraph_index]["x"], dtype=float) >= float(args.remove_edge_particles))
        # identify if particle is within removal bounds from edges in y dir
        acceptance_mask_y = np.logical_and(np.array(particles[ugraph_index]["y"], dtype=float) <= float(args.image_y_pixels) - float(args.remove_edge_particles), np.array(particles[ugraph_index]["y"], dtype=float) >= float(args.remove_edge_particles))
        ...
        if args.debug:
            print('Truth particles in ugraph {} should have {} of {} particles remaining after edge particles removed'.format(ugraph_index, len(acceptance_mask[acceptance_mask==True]), len(acceptance_mask)))

        # now apply the mask to remove particle entries near boundary
        particles[ugraph_index]["molecule"] = np.array(particles[ugraph_index]["molecule"], dtype=str)[acceptance_mask].tolist()
        particles[ugraph_index]["x"] = np.array(particles[ugraph_index]["x"], dtype=float)[acceptance_mask].tolist()
        particles[ugraph_index]["y"] = np.array(particles[ugraph_index]["y"], dtype=float)[acceptance_mask].tolist()
        particles[ugraph_index]["z"] = np.array(particles[ugraph_index]["z"], dtype=float)[acceptance_mask].tolist()
        particles[ugraph_index]["phi"] = np.array(particles[ugraph_index]["phi"], dtype=float)[acceptance_mask].tolist()
        particles[ugraph_index]["theta"] = np.array(particles[ugraph_index]["theta"], dtype=float)[acceptance_mask].tolist()
        particles[ugraph_index]["psi"] = np.array(particles[ugraph_index]["psi"], dtype=float)[acceptance_mask].tolist()
...
def remove_edge_picked_particles(args, picked_particles: dict)->dict:
    for ugraph_index in picked_particles.keys():
        # identify if particle is within removal bounds from edges in x dir
        acceptance_mask_x = np.logical_and(np.array(picked_particles[ugraph_index]["CoordinateX"], dtype=float) <= float(args.image_x_pixels) - float(args.remove_edge_particles), np.array(picked_particles[ugraph_index]["CoordinateX"], dtype=float) >= float(args.remove_edge_particles))
        # identify if particle is within removal bounds from edges in y dir
        acceptance_mask_y = np.logical_and(np.array(picked_particles[ugraph_index]["CoordinateY"], dtype=float) <= float(args.image_y_pixels) - float(args.remove_edge_particles), np.array(picked_particles[ugraph_index]["CoordinateY"], dtype=float) >= float(args.remove_edge_particles))
        # need both x and y coordinates of particle to be okay to keep it!
        acceptance_mask = np.logical_and(acceptance_mask_x, acceptance_mask_y)

        if args.debug:
            print('Picked particles in ugraph {} should have {} of {} particles remaining after edge particles removed'.format(ugraph_index, len(acceptance_mask[acceptance_mask==True]), len(acceptance_mask)))

        # now apply the mask to remove particle entries near boundary
        picked_particles[ugraph_index]["CoordinateX"] = np.array(picked_particles[ugraph_index]["CoordinateX"], dtype=float)[acceptance_mask].tolist()
        picked_particles[ugraph_index]["CoordinateY"] = np.array(picked_particles[ugraph_index]["CoordinateY"], dtype=float)[acceptance_mask].tolist()
        picked_particles[ugraph_index]["AutopickFigureOfMerit"] = np.array(picked_particles[ugraph_index]["AutopickFigureOfMerit"], dtype=float)[acceptance_mask].tolist()
        picked_particles[ugraph_index]["ClassNumber"] = np.array(picked_particles[ugraph_index]["ClassNumber"], dtype=int)[acceptance_mask].tolist()
        picked_particles[ugraph_index]["AnglePsi"] = np.array(picked_particles[ugraph_index]["AnglePsi"], dtype=float)[acceptance_mask].tolist()
        picked_particles[ugraph_index]["ImageName"] = np.array(picked_particles[ugraph_index]["ImageName"], dtype=str)[acceptance_mask].tolist()
        picked_particles[ugraph_index]["MicrographName"] = np.array(picked_particles[ugraph_index]["MicrographName"], dtype=str)[acceptance_mask].tolist()
        picked_particles[ugraph_index]["OpticsGroup"] = np.array(picked_particles[ugraph_index]["OpticsGroup"], dtype=int)[acceptance_mask].tolist()
        picked_particles[ugraph_index]["CtfMaxResolution"] = np.array(picked_particles[ugraph_index]["CtfMaxResolution"], dtype=float)[acceptance_mask].tolist()
        picked_particles[ugraph_index]["CtfFigureOfMerit"] = np.array(picked_particles[ugraph_index]["CtfFigureOfMerit"], dtype=float)[acceptance_mask].tolist()
        picked_particles[ugraph_index]["DefocusU"] = np.array(picked_particles[ugraph_index]["DefocusU"], dtype=float)[acceptance_mask].tolist()
        picked_particles[ugraph_index]["DefocusV"] = np.array(picked_particles[ugraph_index]["DefocusV"], dtype=float)[acceptance_mask].tolist()
        picked_particles[ugraph_index]["DefocusAngle"] = np.array(picked_particles[ugraph_index]["DefocusAngle"], dtype=float)[acceptance_mask].tolist()
        picked_particles[ugraph_index]["CtfBfactor"] = np.array(picked_particles[ugraph_index]["CtfBfactor"], dtype=float)[acceptance_mask].tolist()
        picked_particles[ugraph_index]["CtfScalefactor"] = np.array(picked_particles[ugraph_index]["CtfScalefactor"], dtype=float)[acceptance_mask].tolist()
        picked_particles[ugraph_index]["PhaseShift"] = np.array(picked_particles[ugraph_index]["PhaseShift"], dtype=float)[acceptance_mask].tolist()
...
def run_main(args):
    ...
    # if user wants to remove boundary truth particles, do this now (at the start!)
    if args.remove_edge_particles:
        if args.debug:
            print('Removing particles within {} pixels of image edge from truth particles dict'.format(args.remove_edge_particles))
        # check the user isn't trying to make a margin larger than half ugraph width or height
        if args.remove_edge_particles>(args.image_x_pixels/2) or args.remove_edge_particles>(args.image_y_pixels/2):
            print("Removing all image pixels due to --remove_edge_particles={}\nWhere image has\nwidth of {} pixels\nheight of {} pixels\nExiting!".format(args.remove_edge_particles, args.image_x_pixels, args.image_y_pixels))
            exit(1)
    ...
    if args.picked_particle_file is not None:
        ...
        # if user wants to remove boundary picked particles, do this now (as soon as the starfile is loaded in!)
        if args.remove_edge_particles:
            if args.debug:
                print('Removing particles within {} pixels of image edge from picked particles dict'.format(args.remove_edge_particles))
                for ugraph_index in picked_particles.keys():
                    print('Micrograph {} has {} particles before edge particles removed'.format(ugraph_index, len(picked_particles[ugraph_index]["CoordinateX"])))
            picked_particles = remove_edge_picked_particles(args, picked_particles)
            if args.debug:
                for ugraph_index in picked_particles.keys():
                    print('Micrograph {} has {} particles after edge particles removed'.format(ugraph_index, len(picked_particles[ugraph_index]["CoordinateX"])))
            # also save a new extract (particles.star) starfile which does not contain the particles near the edge
            # for which you gotta load the optics info also
            optics = load_optics_from_starfile(args, args.picked_particle_file)
            if args.debug:
                print('Optics informations read in from {} as:\n{}'.format(args.picked_particle_file, optics))
            noedge_extract_starfile = ExtractStarFile(optics, picked_particles)
            # save the edge-removed particles starfile
            noedge_extract_starfile.write_extract_file(args.remove_edge_particles_starfile)
            # validate it
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help="Number of pixels from edge of image (along x and y axes) to remove particle labels from and write out another truth yaml and picked yaml and picked starfile",
        type=int,
        default=None,
        required=False
    )
    ...
    parser.add_argument(
        ...
        help="File name for the star file to output with only the particles remaining after --remove_edge_particles is applied",
        type=str,
