...
def add_arguments(parser):
    ...
    parser.add_argument(
        "--mrc-dir",
        help="directory with .mrc files and .yaml config files. Overwrites the log file's path",
        type=str,
        default=None,
    )
...
def read_from_config(args)->Tuple[dict, int]:
    ## open each of the config files and read the position and orientation information from them
    
    particles = {} # indexed by micrograph number (starts at args.index_start_value)
    for filename in os.listdir(args.mrc_dir):
        if filename.endswith(".yaml"):
            config = yaml.safe_load(open(os.path.join(args.mrc_dir, filename)))
            micrograph_ind = int(filename.split(".")[0])
            ...
            for molecules in config["sample"]["molecules"]["local"]:
                ...
                for instance in molecules["instances"]:
                    ...
                    particles[str(micrograph_ind).zfill(6)] = {}
                    particles[str(micrograph_ind).zfill(6)]["molecule"] = f
                    particles[str(micrograph_ind).zfill(6)]["x"] = [position[0]]
                    particles[str(micrograph_ind).zfill(6)]["y"] = [position[1]]
                    particles[str(micrograph_ind).zfill(6)]["z"] = [position[2]]
                    particles[str(micrograph_ind).zfill(6)]["phi"] = [orientation[0]]
                    particles[str(micrograph_ind).zfill(6)]["theta"] = [orientation[1]]
                    particles[str(micrograph_ind).zfill(6)]["psi"] = [orientation[2]]
...
def main(args):
    ...
    if args.mrc_dir is not None:
        particles, n_micrographs = read_from_config(args)
    else:
